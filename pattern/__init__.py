...
class Sentiment(lazydict):
    ...
    def __call__(self, s, negation=True, **kwargs):
        ...
        return Score(polarity = avg(map(lambda w_p_s_x: (w_p_s_x[0], w_p_s_x[1]), a), weight),
                 subjectivity = avg(map(lambda w_p_s_x: (w_p_s_x[0], w_p_s_x[2]), a), weight),
                  assessments = a)
...
def language(s):
    ...
    return max(p.items(), key=lambda k_v: (k_v[1], int(k_v[0] == "en")))
